#!/usr/bin/env python3
"""
Plot Gaia multiplicity status of near-cluster non-doubles on top of UMAP 2D map.
...
def load_inputs(embedding_csv: Path, mult_csv: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    emb["source_id"] = emb["source_id"].astype("int64")
    mul["source_id"] = mul["source_id"].astype("int64")
    return emb, mul
...
def plot_status_overlay(emb: pd.DataFrame, mul: pd.DataFrame, status_col: str, out_png: Path, title: str) -> None:
    ...
    markers = {"NC_A_0plus1": "o", "NC_B_2": "s"}

    for typ in ["NC_A_0plus1", "NC_B_2"]:
...
def plot_status_panels(emb: pd.DataFrame, mul: pd.DataFrame, status_col: str, out_png: Path, title: str) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(14, 5.8), sharex=True, sharey=True)
    colors = _status_palette()

    for ax, typ in zip(axes, ["NC_A_0plus1", "NC_B_2"]):
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Gaia multiplicity status on UMAP map for near-cluster non-doubles")
    ap.add_argument(
        "--embedding_csv",
    ...
    ap.add_argument(
        "--multiplicity_csv",
        default="/data/yn316/Codes/output/experiments/embeddings/double_stars_8d/umap_standard/gaia_multiplicity_check_non_cluster/gaia_multiplicity_per_source_clear.csv",
    )
    ap.add_argument(
        "--out_plot_dir",
        default="/data/yn316/Codes/plots/qa/embeddings/double_stars_8d/gaia_multiplicity_check_non_cluster",
    )
    args = ap.parse_args()
    ...
    plot_status_overlay(
        ...
        title="Gaia multiplicity on UMAP for near-cluster non-doubles (<=1.0 arcsec)",
    )
    plot_status_overlay(
        emb,
        mul,
        ...
        title="Gaia multiplicity on UMAP for near-cluster non-doubles (<=2.0 arcsec)",
    )
    plot_status_panels(
        emb,
        mul,
        ...
        title="Gaia multiplicity by near-cluster type (<=1.0 arcsec)",
    )
