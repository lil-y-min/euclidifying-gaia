#!/usr/bin/env python3
"""
Query Gaia DR3 around cluster-type sources to test whether Gaia resolves one vs two+ sources.
...
Outputs:
- output/experiments/embeddings/double_stars_8d/umap_standard/gaia_multiplicity_check/
...
def query_gaia_neighbors(tab: pd.DataFrame, max_radius_arcsec: float, conn: Dict[str, str]) -> pd.DataFrame:
    ...
    # sqlutilpy fails on object dtype string arrays; force fixed-width unicode.
...
def plot_presence_bars(summary: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.title("Gaia multiplicity by cluster type")
...
def plot_nn_hist(df: pd.DataFrame, out_png: Path) -> None:
    plt.figure(figsize=(8, 4.8))
    for t, color in [("A_0plus1", "#d62828"), ("B_2", "#1d3557")]:
    ...
    plt.ylabel("Density")
    plt.title("Nearest-neighbor Gaia distance distribution")
    plt.legend(frameon=True)
    savefig(out_png)


def main() -> None:
    ap = argparse.ArgumentParser(description="Check Gaia multiplicity around cluster-type sources")
    ...
    ap.add_argument("--out_tab_dir", default="/data/yn316/Codes/output/experiments/embeddings/double_stars_8d/umap_standard/gaia_multiplicity_check")
    ap.add_argument("--out_plot_dir", default="/data/yn316/Codes/plots/qa/embeddings/double_stars_8d/gaia_multiplicity_check")
    args = ap.parse_args()
    ...
    dbl = pd.read_csv(doubles_csv)
    dbl["double_cluster"] = pd.to_numeric(dbl["double_cluster"], errors="coerce")
    dbl = dbl.dropna(subset=["double_cluster", "source_id"]).copy()
    dbl["double_cluster"] = dbl["double_cluster"].astype(int)

    dbl = dbl[dbl["double_cluster"].isin(a + b)].copy()
    dbl["cluster_type"] = np.where(dbl["double_cluster"].isin(a), "A_0plus1", "B_2")

    coords = load_source_coords(dataset_root)
    tab = dbl.merge(coords[["source_id", "ra", "dec", "field_tag"]], on="source_id", how="left")
    tab = tab.dropna(subset=["ra", "dec"]).copy()

    # Always write a query pack so the same set can be queried externally if needed.
    ...
    print(f"Querying Gaia neighbors for {len(tab)} sources...")
    try:
